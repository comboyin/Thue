	...
	return {
		...
		init : function () {
			...
			function chonChoDuyet(){
				...
				var dangChoDuyet_td = $("#TableThueMonBai td span");
			...
			function duyetThueMonBai(){
				
				var row = $("#TableThueMonBai input.check_item:checked").parents('tr');
				if(row.length==0){
			...
			$("#dpGhiSo").datepicker();
			_KyThueGhiSo = $.datepicker.formatDate("yy", new Date());
			...
			if ($("#kythue").val() == "") {
				var today = new Date();
				_KyThueThueMonBai = $.datepicker.formatDate("yy", today);
				$("#dpThueMonBai").datepicker('setValue', _KyThueThueMonBai);
			}
			
			
			
			$('#dpGhiSo').datepicker().on('changeDate', function (ev) {
				_KyThueGhiSo = $.datepicker.formatDate("yy", ev.date);
				LoadDSDKThueMonBai();
			});
			
			function LoadDSDKThueMonBai(){
				$("img.loading_GhiSo").css("display","inline");
				...
				$.post('DSDKThueMonBai',data,function(json){
			...
			function LoadDSThueMonBai(){
				$('img.loading').css('display','inline');
				
				/*$("#progess_dpmonths").css('display', 'block');*/
				deleteAllRows();
				//post
				$.get('dsThueMonBai', {Nam : _KyThueThueMonBai},
					function (json) {
			...
			$('#dpThueMonBai').datepicker({
					    format: " yyyy",
					    viewMode: "years", 
					    minViewMode: "years"
				}).on('changeDate', function (ev) {

				_KyThueThueMonBai = $.datepicker.formatDate("yy", ev.date);
				LoadDSThueMonBai();
			...
			if ($("#kythue").val() == "") {
				var today = new Date();
				_KyThueThueTruyThu = $.datepicker.formatDate("yy", today);
				$("#kythue").val(_KyThueThueTruyThu);
				$("#dpThueMonBai").datepicker('setValue', _KyThueThueTruyThu);
			...
			// Khởi tạo oTable
			oTableThueKhoan = $('#TableThueMonBai')
				.dataTable({
			...
			function editRow(oTable, nRow) {
				...
				jqTds[4].innerHTML = '<input style="width:80px;" name="TenGoi" type="text"   value="'
					 + aData[4] + ' "disabled>';
				jqTds[5].innerHTML = '<input style="width:100px;" name="DoanhSo" type="text"  value="'
					 + aData[5] + '">';


				jqTds[6].innerHTML = '<input style="width:100px;" name="NgayPhaiNop" type="text"  value="'
					 + aData[6] + '">';
			...
			jQuery('#TableThueMonBai_wrapper .dataTables_filter input')
			.addClass(" medium"); // modify

			jQuery('#TableThueMonBai_wrapper .dataTables_length select')
			...
			$("#xoa_nhieu").click(function (e) {
				var checkboxs = $("#TableThueMonBai input.check_item:checked").parents('tr');
			...
			$("#check_all").click(function (e) {
				...
				if ($("#check_all").attr("checked") == "checked") {
					$("#TableThueMonBai input.check_item").each(function () {
						$(this).attr('checked', true);
					});
				} else {
					$("#TableThueMonBai input.check_item").each(function () {
			...
			$('#TableThueMonBai a.cancel').live('click', function (e) {
				e.preventDefault();
			...
			$('#TableThueMonBai a.Delete').live('click', function (e) {
				e.preventDefault();
			...
			// dialogTable
			$('#TableThueMonBai button.DialogNNT').live('click', function (e) {
				//lấy dòng được chọn
				var nRow = $(this).parents('tr')[0];
			...
			$('#TableThueMonBai button.DialogTieuMuc').live('click', function (e) {
